...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_norm=None, qk_scale=None, attn_drop=0., proj_drop=0., window_size=None, attn_head_dim=None):
        super().__init__()
    ...
    def forward(self, x, rel_pos_bias=None, return_attention=False, return_qkv=False):
        ...
        if return_qkv: return x, qkv
        return x

class Block(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_norm=None, qk_scale=None, drop=0., attn_drop=0., drop_path=0., init_values=None, act_layer=nn.GELU, norm_layer=nn.LayerNorm, window_size=None, attn_head_dim=None):
        super().__init__()
        self.norm1 = norm_layer(dim)
        self.attn = Attention(dim, num_heads=num_heads, qkv_bias=qkv_bias, qk_norm=qk_norm, qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop, window_size=window_size, attn_head_dim=attn_head_dim)
...
class AblatedNeuralTransformer(nn.Module):
    def __init__(self, EEG_size=1600, patch_size=200, in_chans=1, out_chans=8, num_classes=1000, embed_dim=200, depth=12,
                 ...
                 use_abs_pos_emb=True, use_rel_pos_bias=False, use_mean_pooling=True, init_scale=0.001, **kwargs):
        super().__init__()
        ...
        # FIXED: Exact architectural parity with original LaBraM dimensions
        if use_abs_pos_emb:
        ...
        self.blocks = nn.ModuleList([
            Block(
                ...
                drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[i], norm_layer=norm_layer, init_values=init_values, window_size=None)
    ...
    def forward_features(self, x, input_chans=None, return_patch_tokens=False, return_all_tokens=False, **kwargs):
        ...
        # FIXED: Removed the input_chans logic to strictly match original LaBraM
        if self.pos_embed is not None:
            pos_embed = self.pos_embed[:, 1:, :].unsqueeze(2).expand(batch_size, -1, input_time_window, -1).flatten(1, 2)
...
# Model Registration matching the timm arguments
@register_model
def ablated_labram_base_patch200_200(pretrained=False, **kwargs):
    model = AblatedNeuralTransformer(
...
@register_model
def ablated_labram_large_patch200_200(pretrained=False, **kwargs):
    model = AblatedNeuralTransformer(
