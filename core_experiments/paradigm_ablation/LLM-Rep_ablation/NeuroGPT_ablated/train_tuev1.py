...
from safetensors.torch import load_model  # Needed for .safetensors

'''
CUDA_VISIBLE_DEVICES=0 nohup torchrun --nproc_per_node=2 --master_port=29534 train_tuev1.py --pretrained-model none --ft-only-encoder False --training-style decoding --run-name "NeuroGPT_Strict_LLMRep_Ablation1" --log-dir "results_tuev_llmrep_ablation/" --per-device-training-batch-size 32 --per-device-validation-batch-size 32 > llmrep_tuev.log 2>&1 &
...
from batcher.downstream_dataset import TUEVDataset
...
def make_model(model_config):
    """Builds the model architecture and safely handles weight loading"""
    ...
    # Activates the bypass switch. If --ft-only-encoder is True, 
    ...
    pretrained_path = model_config.get("pretrained_model")
    if pretrained_path and pretrained_path != 'none':
        ...
        if str(pretrained_path).endswith(".safetensors"):
            ...
            load_model(model, pretrained_path, strict=False)
        else:
            # Load using standard torch; weights_only=True handles the warning
...
def run_single_seed_training(config, seed):
    ...
    config["log_dir"] = seed_log_dir
    os.makedirs(seed_log_dir, exist_ok=True)

    files_path = config["dst_data_path"] 
    train_path = os.path.join(files_path, 'processed_train/')
    val_path = os.path.join(files_path, 'processed_eval/')
    test_path = os.path.join(files_path, 'processed_test/')
    ...
    train_dataset = TUEVDataset(root=train_path, filenames=train_files, **dataset_kwargs)
    validation_dataset = TUEVDataset(root=val_path, filenames=val_files, **dataset_kwargs)
    test_dataset = TUEVDataset(root=test_path, filenames=test_files, **dataset_kwargs)
    ...
    trainer = make_trainer(
        ...
        run_name=f"{config['run_name']}_seed_{seed}",
        ...
        seed=seed,
        fp16=config["fp16"],
        report_to="none"  # Silences W&B deprecation warning
    )
    ...
    # Save raw state_dict for manual reproducibility
    ...
    print(f"Seed {seed} Test Results: Balanced Acc: {test_results.get('balanced_accuracy', 0):.4f}")
...
def get_args():
    ...
    parser.add_argument('--dst-data-path', default="/homes/xw2336/data_portal/TUEV/processed", type=str)
    
    # -------------------------------------------------------------------------
    # ABLATION DEFAULTS: 
    # Changed to 'none' and 'True' to strictly enforce LLM-Rep Ablation.
    # -------------------------------------------------------------------------
    parser.add_argument('--pretrained-model', type=str, default='none')
    parser.add_argument('--ft-only-encoder', default='True', type=str)
    ...
    # TUEV specific mapping (6 classes)
    parser.add_argument('--num-decoding-classes', default=6, type=int)
    ...
    # Output and logging specific defaults for the ablation
    parser.add_argument('--log-dir', default='results_tuev_llmrep_ablation/', type=str)
    parser.add_argument('--run-name', default='NeuroGPT_Strict_LLMRep_Ablation_TUEV', type=str)
...
if __name__ == '__main__':
    ...
    # Process booleans and none-strings
    for arg in config:
    ...
    if 'balanced_accuracy' in df.columns:
        b_acc = df['balanced_accuracy'].values
        f1 = df['f1'].values if 'f1' in df.columns else [0]
        kappa = df['kappa'].values if 'kappa' in df.columns else [0]
        
        print("\n" + "="*50)
        print(f"FINAL TUEV LLM-REP ABLATION RESULTS ({len(SEEDS)} SEEDS)")
        print("="*50)
