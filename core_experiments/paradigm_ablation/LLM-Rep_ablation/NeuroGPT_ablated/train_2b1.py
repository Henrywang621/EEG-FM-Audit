...
def make_model(model_config: Dict=None):
    ...
    # Replaced the bug that forced ft_encoder_only=False. Now properly tracks 
    # the config to extract the Conformer and ignore the LLM.
    # =========================================================================
...
def get_args() -> argparse.ArgumentParser:
    ...
    # -------------------------------------------------------------------------
    # ABLATION DEFAULTS: 
    # Forced 'none', 'True', and 'decoding' to strictly run LLM-Rep Ablation.
    # -------------------------------------------------------------------------
    parser.add_argument('--pretrained-model', metavar='DIR', type=str, default='none')
    parser.add_argument('--ft-only-encoder', metavar='BOOL', default='True', choices=('True', 'False'), type=str)
    parser.add_argument('--training-style', metavar='STR', default='decoding', choices=('CSM', 'CSM_causal', 'decoding'), type=str)
    ...
    # Output and logging defaults for the BCI IV 2b ablation
    parser.add_argument('--log-dir', metavar='DIR', type=str, default='results_bci2b_llmrep_ablation/')
    parser.add_argument('--run-name', metavar='STR', type=str, default='NeuroGPT_Strict_LLMRep_Ablation_BCI2b')
