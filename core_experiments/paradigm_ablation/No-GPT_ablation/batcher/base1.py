...
class EEGDataset(Dataset):
    ...
    def split_chunks(self, data, length=500, ovlp=50, num_chunks=10, start_point=-1):
      ...
      # --- NEUROGPT STANDARD: Discard incomplete chunks ---
    ...
    def preprocess_sample(self, sample, seq_len, labels=None) -> dict:
        """
        Standardizes input for Neuro-GPT. Verified for TUAB, TUEV, and BCI IV 2b.
        """
        out = {}
        
        # 1. Resample to 128 Hz (Neuro-GPT Hardware Standard)
        # This prevents spectral "frequency shifting" in the model's perception.
        # num_samples_target = int(sample.shape[1] * self.target_fs / self.original_fs)
        # sample = signal.resample(sample, num_samples_target, axis=-1)

        # 2. Artifact Clipping at ±800 μV
        # Crucial for clinical data to prevent Z-score variance distortion.
        sample = np.clip(sample, -800, 800) 

        # 3. Z-Score Normalization
        if self.do_normalization:
            means = np.mean(sample, axis=-1, keepdims=True)
            stds = np.std(sample, axis=-1, keepdims=True) + 1e-6
            sample = (sample - means) / stds

        # 4. Chunking (Resulting shape: [seq_len, 22, 128])
        chunks, seq_on = self.split_chunks(sample, self.chunk_len, self.ovlp, seq_len, 0)

        # 5. Padding & Masking for Transformer
        # attention_mask = np.ones(seq_len)
        attention_mask = np.ones(len(chunks))
        chunks = self._pad_seq_right_to_n(seq=chunks, n=seq_len, pad_value=0)
        attention_mask = self._pad_seq_right_to_n(seq=attention_mask, n=seq_len, pad_value=0)

        # 6. Flattening for Linear Projection
        if self.gpt_only:
            # Reshape [Seq, Ch, Time] -> [Seq, Ch * Time] (e.g., 22 * 128 = 2816)
            chunks = np.reshape(chunks, (seq_len, chunks.shape[1] * chunks.shape[2]))
