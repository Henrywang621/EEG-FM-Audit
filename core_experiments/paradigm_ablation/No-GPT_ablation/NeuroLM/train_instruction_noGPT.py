...
from model.model_neurolm1 import NeuroLM
# from model.model import GPTConfig
from pathlib import Path
import tiktoken
from utils import prepare_TUAB_dataset, prepare_CCD_dataset, prepare_TUEV_dataset, prepare_TUSL_dataset, prepare_HMC_dataset, prepare_Workload_dataset, cosine_scheduler, get_metrics
...
def get_instruct_datasets(args, downstream_dataset: str, eeg_max_len=-1, text_max_len=-1):
        ...
        if downstream_dataset == 'SEED':
            ...
            dataset_train, dataset_test, dataset_val = prepare_TUAB_dataset(Path(args.dataset_dir, 'THUA/v3.0.1/edf/processed'), is_instruct=True, 
        ...
        elif downstream_dataset == 'CCD':
            dataset_train, dataset_test, dataset_val = prepare_CCD_dataset(Path(args.dataset_dir, '/home/u5s/henrywang.u5s/HBN_dataset/processed'), is_instruct=True, 
                                                                            eeg_max_len=eeg_max_len, text_max_len=text_max_len)
            ...
            dataset_train, dataset_test, dataset_val = prepare_TUEV_dataset(Path(args.dataset_dir, 'TUEV/v2.0.1/edf/processed'), is_instruct=True, 
                                                                            eeg_max_len=eeg_max_len, text_max_len=text_max_len)
...
def main(args):
    ...
    checkpoint_out_dir = os.path.join(args.out_dir, 'checkpoints/instruction-B5')
    if master_process:
    ...
    # for name in ['TUAB', 'TUEV', 'CCD']:
    for name in ['TUAB', 'TUEV']:
    ...
    if init_from == 'resume':
        ...
        model = NeuroLM(tokenizer_ckpt_path=tokenizer_ckpt_path)
    ...
    for epoch in range(start_epoch, args.epochs):
        for dataset_info in datasets:
            ...
            for step, (batch) in enumerate(dataset_info['data_loader_train']):
                ...
                Y_eeg = torch.full((X_eeg.size(0), X_eeg.size(1)), fill_value=-1-50257).to(device, non_blocking=True)
...
def get_args():
    ...
    parser.add_argument('--tokenizer_path', default='checkpoints/VQ/ckpt.pt', help='path where tokenizer is')
