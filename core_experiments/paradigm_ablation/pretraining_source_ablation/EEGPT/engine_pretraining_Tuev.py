...
class LitEEGPT(pl.LightningModule):
    ...
    def make_masks(self, num_patchs, mC_x=None, p_n_y=0.5, p_c_y=0.2):
        C, N = num_patchs
        
        # Auto-adjust mC_x based on C
        if mC_x is None:
            mC_x = max(1, C // 2)  # Take half the channels for C=3, mC_x=1
    
        # Ensure mC_x doesn't exceed C
        mC_x = min(mC_x, C - 1)  # Leave at least 1 for mask_y_bx
        ...
        for attempt in range(max_attempts):
            ...
            for i in range(N):
                ...
                if random.random() > p_n_y:
                    ...
                    mask_y_bx.append(c_idx[mC_x:])  # Now has C - mC_x elements
                else:
                    mask_y.append(c_idx)
            ...
            sampled = torch.rand(mask_y_bx.shape) < p_c_y
            if not sampled.any():
                sampled[0] = True  # Force at least one
        
            mask_y_bx = mask_y_bx[sampled]
