...
# This function right-pads a sequence (seq) along the first dimension to a fixed length n.
def _pad_seq_right_to_n(
    ...
    # *seq.shape[1:] unpacks a tuple so that each element becomes a separate positional argument.
...
#     def __getitem__(self, idx):
#         data = pickle.load(open(self.filenames[idx], "rb"))
#         data = data[:22]
#         # data = self.load_tensor(self.filenames[idx])
#         #===reorder channels====
#         data = self.reorder_channels(data)
...
#         # If the data is too short, the actual number of chunks will be re-calculated.
#         if start_point == -1:
#             if num_chunks * length > total_len - 1:
#                 start_point = 0
#                 actual_num_chunks = total_len // length
...
class EEGDataset(Dataset):
    ...
    def reorder_channels(self, data):
        chann_labels = {'C3': 0, 'C4': 1, 'CZ': 2}
        reorder_labels = {'C3': 0, 'CZ': 1, 'C4': 2}
