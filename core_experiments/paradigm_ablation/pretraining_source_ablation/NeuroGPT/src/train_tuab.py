...
def make_model(model_config):
    ...
    if model_config["pretrained_model"] is not None and model_config["pretrained_model"] != 'none':
        print(f"Loading pretrained weights from {model_config['pretrained_model']}")
        model.load_state_dict(torch.load(model_config["pretrained_model"], map_location='cpu'), strict=False)
...
def run_single_seed_training(config, seed):
    ...
    # trainer.evaluate() will use the 'best' model because load_best_model_at_end=True
    raw_test_results = trainer.evaluate(test_dataset)
    
    # Normalize keys: HF adds "eval_" or "test_" prefix. We want to ensure consistency.
    test_results = {}
    ...
    print(f"Seed {seed} Test Results: Balanced Acc: {test_results.get('balanced_accuracy', 0):.4f}, F1: {test_results.get('f1', 0):.4f}")
...
if __name__ == '__main__':
    ...
    # We normalized keys in run_single_seed_training, so these should match exactly
