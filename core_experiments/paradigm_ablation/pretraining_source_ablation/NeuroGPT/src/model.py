...
class Model(torch.nn.Module):
    ...
    # -----------------------------------------------------
    # 1. Load checkpoint (supports .bin or .safetensors)
    # -----------------------------------------------------
        if ext == ".safetensors":
    ...
    def forward(
        ...
        if self.encoder is not None:
            # First let the splitted chunks of raw input through the encoder
            features = self.encoder(batch['inputs'])
            
            # --- TENSOR SHAPE FIX ---
            if features.dim() == 2:
                if self.is_decoding_mode and self.ft_only_encoder:
                    # Reshape [Batch, Classes] -> [Batch, 1, Classes] 
                    # This prevents the embedder from misinterpreting the class dimension as a sequence length
                    features = features.unsqueeze(1)
                else:
                    # Default behavior for standard pretraining
                    features = features.unsqueeze(-1)
            # ------------------------

            # Fine-tune only the encoder early exit
            if self.is_decoding_mode and self.ft_only_encoder:
