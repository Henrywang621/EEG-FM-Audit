...
class BCICIV2bLoader(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.is_val:
            ...
            valid_text_len = text.size(0)  # <-- Capture length BEFORE padding!
        ...
        if self.eeg_max_len > valid_eeg_len:
            ...
            # Use 'pad' string like TUABLoader
            pad_len = self.eeg_max_len - valid_eeg_len
            ch_names_pad_list = ch_names_list + ['pad'] * pad_len
            input_chans_pad = torch.IntTensor(get_chans(ch_names_pad_list))
            input_time_pad = torch.cat([input_time, torch.zeros(pad_len, dtype=torch.int)])
        else:
            X_eeg = data
        ...
        # Training Labels (Exactly like TUAB)
        Y_text = torch.full_like(text, fill_value=-1)
        prompt_len = self.prompt.size(0)
        
        # Use the valid_text_len we saved earlier!
        Y_text[prompt_len - 1 : valid_text_len - 1] = text[prompt_len : valid_text_len]
