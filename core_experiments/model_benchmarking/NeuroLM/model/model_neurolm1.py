...
class NeuroLM(nn.Module):
    ...
    def __init__(self,
        ...
        self.self_atten = LightDecoder(
            d_model=n_embd,
            n_head=12,
            n_layer=1,
            vocab_size=50304   # binary EEG classification
        )
        ...
        self.n_embd = 768
        self.pos_embed = nn.Embedding(256, self.n_embd)

        # task layer
        self.encode_transform_layer = nn.Sequential(
            nn.Linear(n_embd, self.n_embd),
            nn.GELU(),
        ) if n_embd != self.n_embd else nn.Identity()
    ...
    def forward(self, x_eeg=None, y_eeg=None, x_text=None, y_text=None, input_chans=None, input_time=None, input_mask=None, eeg_mask=None, eeg_text_mask=None):
        ...
        logits, loss, accuracy = self.self_atten(x_eeg, y_eeg, x_text, y_text, input_time)
    ...
    def generate(self, x_eeg, x_text, input_chans, input_time, input_mask, eeg_mask=None, eeg_text_mask=None, max_new_tokens=10, temperature=1.0, top_k=1):
        ...
        for _ in range(max_new_tokens):
            logits, _, _ = self.self_atten(x_eeg=x_eeg, x_text=x_text, eeg_time_idx=input_time)
