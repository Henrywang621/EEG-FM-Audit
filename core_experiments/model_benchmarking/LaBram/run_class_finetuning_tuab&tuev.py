...
def main(args, ds_init):
    ...
    # Enforce strict determinism for reproducible benchmarking
    ...
    # ---------------------------------------------------------
    # MODIFIED: Tracking Balanced Accuracy instead of Accuracy
    # ---------------------------------------------------------
    max_val_bacc = 0.0
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if data_loader_val is not None:
            ...
            print(f"Val Balanced Acc: {val_stats['balanced_accuracy']:.4f} | Test Balanced Acc: {test_stats['balanced_accuracy']:.4f}")
            
            # Save the model and stats when validation balanced accuracy improves
            if max_val_bacc < val_stats["balanced_accuracy"]:
                max_val_bacc = val_stats["balanced_accuracy"]
                best_test_stats_for_seed = test_stats # Capture the corresponding test stats
            ...
            print(f'Max Val Balanced Accuracy: {max_val_bacc:.4f}')
    ...
    # Return the test stats linked to the best validation checkpoint
    return best_test_stats_for_seed


if __name__ == '__main__':
    # Parse base arguments to get the root output directory
    base_opts, base_ds_init = get_args()
    base_output_dir = base_opts.output_dir
    
    # Define the 5 random seeds to iterate over
    seeds = [42, 3407, 6, 16, 66]
    
    # Dictionaries to aggregate performance metrics
    aggregate_results = {
        'balanced_accuracy': [],
        'f1_score': [],
        'cohen_kappa': []
    ...
    for seed in seeds:
        print(f"\n{'='*50}\nStarting Fine-tuning Run with Random Seed: {seed}\n{'='*50}")
        
        opts, ds_init = get_args()
        opts.seed = seed
        
        # Create unique subdirectories for each seed to prevent best models from overwriting one another
        if base_output_dir:
            opts.output_dir = os.path.join(base_output_dir, f"seed_{seed}")
        else:
            opts.output_dir = f"output_seed_{seed}"
            
        Path(opts.output_dir).mkdir(parents=True, exist_ok=True)
        
        # Run the main pipeline and collect the test stats for the best epoch
        best_test_stats = main(opts, ds_init)
        ...
        if best_test_stats is not None:
            ...
            # Handle potential key differences between TUAB (f1) and TUEV (f1_weighted) natively
            f1_val = best_test_stats.get('f1_weighted', best_test_stats.get('f1', 0))
            aggregate_results['f1_score'].append(f1_val)

    # Calculate and Print Final Averaged Results
    print("\n\n" + "="*55)
    print("FINAL TEST PERFORMANCE ACROSS 5 SEEDS (Best Val Checkpoint)")
    print("="*55)
    
    if len(aggregate_results['balanced_accuracy']) > 0:
        bacc_mean = np.mean(aggregate_results['balanced_accuracy'])
        bacc_std = np.std(aggregate_results['balanced_accuracy'])
        
        f1_mean = np.mean(aggregate_results['f1_score'])
        f1_std = np.std(aggregate_results['f1_score'])
        
        kappa_mean = np.mean(aggregate_results['cohen_kappa'])
        kappa_std = np.std(aggregate_results['cohen_kappa'])

        print(f"Balanced Accuracy: {bacc_mean:.4f} ± {bacc_std:.4f}")
        print(f"F1 Score:          {f1_mean:.4f} ± {f1_std:.4f}")
        print(f"Cohen's Kappa:     {kappa_mean:.4f} ± {kappa_std:.4f}")
    else:
        print("No evaluation statistics were returned. Ensure 'disable_eval_during_finetuning' is False.")