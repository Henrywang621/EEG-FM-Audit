...
def evaluate(data_loader, model, device, header='Test:', ch_names=None, metrics=['acc'], is_binary=True):
    ...
    for step, batch in enumerate(metric_logger.log_every(data_loader, 10, header)):
        ...
        # compute output
        # UPDATED: Using the modern torch.amp.autocast syntax
        with torch.amp.autocast('cuda'):
            output = model(EEG, input_chans=input_chans)
            loss = criterion(output, target)

        # 1. Calculate probabilities as Tensors
        if is_binary:
            prob = torch.sigmoid(output)
        else:
            prob = torch.softmax(output, dim=1)
    
        # 2. Append Tensors to lists (Cleanest way for torch.cat later)
        # We detach and move to CPU to free up GPU memory immediately
        pred.append(prob.detach().cpu())
        true.append(target.detach().cpu())

        # 3. Local metrics (Conversion to NumPy here is fine as it's not stored in the lists)
        prob_np = prob.detach().cpu().float().numpy()
        target_np = target.detach().cpu().numpy()

        # --- FIX START ---
        # Force probabilities to sum to 1.0 to satisfy sklearn's strict check
        if not is_binary:
            # Re-normalize across the class dimension (axis 1)
            prob_np = prob_np / prob_np.sum(axis=1, keepdims=True)
        # --- FIX END ---
        results = utils.get_metrics(prob_np, target_np, metrics, is_binary)
    ...
    # 1. Cat and convert
    pred = torch.cat(pred, dim=0).numpy().astype('float64') # Use float64 for better precision
    true = torch.cat(true, dim=0).numpy()

    # 2. Precise normalization
    if not is_binary:
        s = pred.sum(axis=1, keepdims=True)
        # Only divide where sum is not 0 to avoid NaNs
