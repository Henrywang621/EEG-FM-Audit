...
# 🚨 IMPORT MSCFormer & Parameters HERE 🚨
...
class BCICIV2b_Dataset(Dataset):
    ...
    def __init__(self, raw_data_dict, subject_ids,
        ...
        # 🚨 MSCFormer requires resampling to 1000 🚨
...
def train_any_model(config):
    ...
    if model_name == "MSCFormer":
        # 🚨 Build the Parameters object specifically for MSCFormer 🚨
        params = Parameters(dropout_rate=config["dropout_rate"])
        params.f1 = config["f1"]
        params.emb_size = 3 * config["f1"]  # Dynamic embedding calculation
        params.depth = config["depth"]
        params.pooling_size = config["pooling_size"]
        
        model = MSCFormer(params).to(device)
    ...
    # MSCFormer uses BCEWithLogitsLoss
    ...
    for epoch in range(config["epochs"]):
        ...
        for X,y in train_loader:
            ...
            # 🚨 FIX: MSCFormer expects (Batch, 1, Channels, Time) 🚨
            X = X.unsqueeze(1)
            
            _, logits = model(X)
            
            # Requires one-hot encoding for BCE loss
            loss = criterion(logits, F.one_hot(y, 2).float())
            loss.backward()
            optimizer.step()
        ...
        with torch.no_grad():
            for X,y in val_loader:
                ...
                # Convert logits to probabilities using sigmoid for BCE outputs
...
def evaluate_final_model(config,full_train_subjs,test_subj,global_data):
    ...
    save_dir = "MSCFormer_Saved_Models"
    os.makedirs(save_dir, exist_ok=True)
    
    save_path = os.path.join(save_dir, f"MSCFormer_best_model_{test_subj}.pth")
    checkpoint = {
...
def main():
    ...
    # 🚨 EXPANDED MSCFormer SEARCH SPACE 🚨
    search_space = {
        ...
        # --- Tunable MSCFormer Specific Parameters ---
    ...
    print("Starting Nested LOO search for MSCFormer")
    ...
    for i,test_subj in enumerate(all_subjects):
        ...
        analysis = tune.run(
            ...
            name=f"MSCFormer_Nested_LOO_{test_subj}",
        )
    ...
    metric_cols=["accuracy","balanced_accuracy","test_f1_score","auc","kappa"]
    ...
    df.to_csv("MSCFormer_Nested_LOO_results.csv",index=False)
