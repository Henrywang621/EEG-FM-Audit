...
def train_any_model(config):
    ...
    if model_name == "CSPNet":
        model = CSPNet(
            chunk_size=500,                 # Anchored to raw un-resampled data length
            num_electrodes=3,               
            num_classes=2,                  
    ...
    # 🚨 FIX: CSPNet outputs log_softmax, so we must use NLLLoss 🚨
    ...
    for epoch in range(config["epochs"]):
        ...
        for X,y in train_loader:
            ...
            # 🚨 FIX: Squeeze spatial dimensions for CSPNet 🚨
        ...
        with torch.no_grad():
            for X,y in val_loader:
                ...
                # 🚨 FIX: Convert log_softmax to probabilities using torch.exp 🚨
...
def evaluate_final_model(config,full_train_subjs,test_subj,global_data):
    ...
    save_dir = "CSPNet_Saved_Models"
    os.makedirs(save_dir, exist_ok=True)
    
    # Save the model weights AND the winning hyperparameters inside the folder
    save_path = os.path.join(save_dir, f"CSPNet_best_model_{test_subj}.pth")
    checkpoint = {
...
def main():
    ...
    search_space = {
        ...
        # --- Tunable CSPNet Specific Parameters ---
    ...
    print("Starting Nested LOO search for CSPNet")
    ...
    for i,test_subj in enumerate(all_subjects):
        ...
        analysis = tune.run(
            ...
            name=f"CSPNet_Nested_LOO_{test_subj}",
        )
    ...
    df.to_csv("CSPNet_Nested_LOO_results.csv",index=False)
