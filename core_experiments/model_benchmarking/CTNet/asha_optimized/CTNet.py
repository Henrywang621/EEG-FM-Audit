...
class PatchEmbeddingCNN(nn.Module):
    def __init__(self, f1=16, kernel_size=64, D=2, pooling_size1=8, pooling_size2=8, dropout_rate=0.3, number_channel=3, emb_size=40):
        super().__init__()
        f2 = D*f1
...
class PositioinalEncoding(nn.Module):
    def __init__(self, embedding, length=5000, dropout=0.1):
        super().__init__()
        self.dropout = nn.Dropout(dropout)
        self.encoding = nn.Parameter(torch.randn(1, length, embedding))
        
    def forward(self, x): 
        # --- FIX: DYNAMIC DEVICE MATCHING ---
...
class EEGTransformer(nn.Module):
    def __init__(self, heads=4, 
                 ...
                 n_classes=6, # --- FIX: EXPLICITLY ACCEPT N_CLASSES ---
                 **kwargs):
        super().__init__()
        
        # --- FIX: OVERRIDE DEFAULT CLASSES AND CHANNELS ---
        self.number_class = n_classes
        
        if 'in_chans' in kwargs:
            self.number_channel = kwargs['in_chans']
        elif 'number_channel' in kwargs:
            self.number_channel = kwargs['number_channel']
        else:
            _, self.number_channel = numberClassChannel(database_type)
        # -----------------------------------------------
        ...
        self.position = PositioinalEncoding(emb_size, length=max(input_window_samples, 5000), dropout=0.1)
        self.trans = TransformerEncoder(heads, depth, emb_size)
        self.flatten = nn.Flatten()
        
        # This will now correctly map to 6 classes instead of 2
