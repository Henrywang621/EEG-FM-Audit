...
for i in range(1, 10):
    ...
    train_subjs = [s for s in subjects if s != current_subj]
    ...
    # Add checkpoint callback for each subject
    checkpoint_callback = ModelCheckpoint(
        dirpath=f'/users/yyang/EEGPT/checkpoints/subject_{i:02d}/',
        filename='best',
        monitor='valid_loss',
        mode='min',
        save_top_k=1,
    ...
    trainer = pl.Trainer(
        ...
        callbacks=callbacks,
        enable_checkpointing=True,  # Changed to True
        logger=[
            pl_loggers.TensorBoardLogger('/users/yyang/EEGPT/log/', 
                                        name="EEGPT_BCIC2B_tb", 
                                        version=f"subject{i}"),
            pl_loggers.CSVLogger('/users/yyang/EEGPT/log/', 
