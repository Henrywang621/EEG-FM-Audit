...
# If transformers' seed_worker has a different signature, wrap/replace it.
def _local_seed_worker(worker_id):
    """Single-arg worker_init_fn compatible with DataLoader."""
    seed = torch.initial_seed() % 2**32
    random.seed(seed + worker_id)
    np.random.seed(seed + worker_id)

# Replace the imported name with the safe local one
seed_worker = _local_seed_worker
    ...
    def get_eval_dataloader(self, eval_dataset: Optional[Dataset] = None) -> DataLoader:
        ...
        if isinstance(eval_dataset, torch.utils.data.IterableDataset):
            if self.args.world_size > 1:
                ...
                #In distributed setups, each process must handle only its own portion of the data. 
                #Otherwise, every GPU would process the same samples, duplicating work and giving 
                # incorrect metrics.
