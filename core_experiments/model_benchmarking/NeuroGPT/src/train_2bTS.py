...
import sys
from safetensors.torch import load_model  # <-- IMPORT FOR SAFETENSORS
...
def train(config: Dict=None):
    ...
    # Allow custom epochs from command line, default to 50
    num_fixed_epochs = config.get("num_train_epochs", 50) 
...
def make_model(model_config: Dict=None):
    if model_config["use_encoder"] == True:
        chann_coords = None
        # Ensures mapping to the 3-channel standard for BCI 2b
        encoder = EEGConformer(n_outputs=model_config["num_decoding_classes"], n_chans=3, n_times=model_config['chunk_len'], ch_pos=chann_coords, is_decoding_mode=model_config["ft_only_encoder"])
        ...
        encoder = None
        model_config["parcellation_dim"] = model_config["chunk_len"] * 3
    ...
    # --- SHAPE MISMATCH PATCH ---
    # Automatically strip 22-channel zero-padding down to the 3 active BCI 2b channels
    if model_config.get("use_encoder", False) and encoder is not None:
        original_encoder_forward = model.encoder.forward
        def patched_encoder_forward(x):
            # Handle (Batch, 1, Channels, Time)
            if x.dim() == 4 and x.shape[2] == 22:
                sums = torch.abs(x).sum(dim=(0, 1, 3))
                active_idx = torch.nonzero(sums).squeeze()
                if active_idx.numel() == 3:
                    x = x[:, :, active_idx, :]
                else:
                    x = x[:, :, :3, :] # Fallback
            # Handle (Batch, Channels, Time)
            elif x.dim() == 3 and x.shape[1] == 22:
                sums = torch.abs(x).sum(dim=(0, 2))
                active_idx = torch.nonzero(sums).squeeze()
                if active_idx.numel() == 3:
                    x = x[:, active_idx, :]
                else:
    ...
    if model_config.get("ft_only_encoder", False):
    ...
    # --- UPDATED LOADING LOGIC FOR SAFETENSORS ---
    ...
    if pretrained_path and pretrained_path != 'none':
        ...
        elif str(pretrained_path).endswith(".pth") or str(pretrained_path).endswith(".pt"):
            state_dict = torch.load(pretrained_path, map_location='cpu', weights_only=True)
            model.load_state_dict(state_dict, strict=False)
        else:
            model.from_pretrained(pretrained_path)

    if model_config.get("freeze_embedder"):
        for param in model.embedder.parameters():
            param.requires_grad = False

    if model_config.get("freeze_decoder"):
        for param in model.decoder.parameters():
            param.requires_grad = False

    if model_config.get("freeze_encoder"):
    ...
    if model_config.get("freeze_decoder_without_pooler_heads"):
    ...
    if model_config.get("freeze_unembedder") and unembedder is not None:
...
def get_args() -> argparse.ArgumentParser:
    ...
    parser.add_argument('--num-decoding-classes', metavar='INT', default=4, type=int)
    
    # Epochs & Steps
    parser.add_argument('--num_train_epochs', default=50, type=int)  # <-- EPOCHS ARG ADDED HERE
