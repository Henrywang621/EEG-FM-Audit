...
def extract_logits(outputs):
    """
    Specifically extracts classification logits from NeuroGPT.
    Prioritizes 'decoding_logits' as identified in user logs.
    """
    if torch.is_tensor(outputs):
        return outputs
    
    if isinstance(outputs, dict):
        # Prioritize 'decoding_logits' based on your specific model output
        for key in ['decoding_logits', 'logits', 'out', 'output', 'y_hat']:
            if key in outputs:
                return outputs[key]
        
        # Fallback if preferred keys aren't found
        available_keys = list(outputs.keys())
        return outputs[available_keys[0]]
...
def run_neurogpt_analysis():
    ...
    BASE_LOG_DIR = "/homes/xw2336/data_portal/LLM_eva_fast/NeuroGPT/pretrained_model/tuab_original" 
    ...
    for m_seed in MODEL_SEEDS:
        ...
        with torch.no_grad():
            for batch in tqdm(loader, desc=f"Baseline Model {m_seed}"):
                ...
                # Ensure logits are [Batch, Classes] and take second class prob
                probs = torch.softmax(logits, dim=-1)
                p_orig.extend(probs[:, 1].cpu().numpy().flatten())
        ...
        m_f1 = f1_score(y_true, pred_orig, average='weighted') * 100
        ...
        for p_seed in PHASE_SHUFFLE_SEEDS:
            ...
            with torch.no_grad():
                for batch in tqdm(loader, desc=f"  Shuffle {p_seed}", leave=False):
                    ...
                    p_rand_probs.extend(probs_rand[:, 1].cpu().numpy().flatten())

            p_rand_probs = np.array(p_rand_probs)
            pred_rand = (p_rand_probs > 0.5).astype(float)
            
            for k, v in [("bacc", m_bacc), ("f1", m_f1), ("kappa", m_kappa)]: 
                results["orig"][k].append(v)
            
            results["rand"]["bacc"].append(balanced_accuracy_score(y_true, pred_rand) * 100)
            results["rand"]["f1"].append(f1_score(y_true, pred_rand, average='weighted') * 100)
            results["rand"]["kappa"].append(cohen_kappa_score(y_true, pred_rand))

        del model; torch.cuda.empty_cache()

    # Final Summary
    print("\n" + "="*75 + "\nNEUROGPT ROBUSTNESS SUMMARY (50 RUNS)\n" + "="*75)
