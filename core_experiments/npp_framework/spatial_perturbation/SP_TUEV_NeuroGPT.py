...
# Standard TUH EEG Corpus channel order
TUH_CH_NAMES = ['EEG FP1', 'EEG FP2', 'EEG F3', 'EEG F4', 'EEG C3', 'EEG C4', 'EEG P3', 'EEG P4', 'EEG O1', 'EEG O2', 
...
def apply_spatial_noise(x, target_indices, noise_lambda=1.0):
    """Applies noise to specific channels in (B, Chunks, C, T) input."""
...
def run_analysis():
    ...
    BASE_DIR = "/homes/xw2336/data_portal/LLM_eva_fast/NeuroGPT/pretrained_model/tuev_original" 
    ...
    print("\n" + "="*85 + "\nNEUROGPT TUEV SPATIAL PERTURBATION SUMMARY\n" + "="*85)
    
    # --- ADDED: BASELINE PERFORMANCE SUMMARY ---
    b_orig = res_orig["bacc"]
    f_orig = res_orig["f1"]
    k_orig = res_orig["kappa"]
    print(f"\n[BASELINE (NO NOISE)]")
    print(f"BACC : {np.mean(b_orig):.2f} ± {np.std(b_orig):.2f}")
    print(f"F1   : {np.mean(f_orig):.2f} ± {np.std(f_orig):.2f}")
    print(f"Kappa: {np.mean(k_orig):.3f} ± {np.std(k_orig):.3f}\n")
    print("-" * 85)
    # ------------------------------------------
