...
# ROI DEFINITIONS
# ============================================================================
# 17 channels used: FP1, FP2, F3, F4, C3, C4, P3, P4, O1, O2, F7, F8, FZ, CZ, PZ, FPZ, OZ
ROI_DEFINITIONS = {
    'Frontal': ['FP1', 'FP2', 'FPZ', 'F3', 'F4', 'F7', 'F8', 'FZ'],
    'Central': ['C3', 'CZ', 'C4'],
    'Temporal': [],  # No temporal channels in the 17 channels (T3, T4, T5, T6 excluded)
    'Parietal_Occipital': ['P3', 'PZ', 'P4', 'O1', 'OZ', 'O2']
}
...
class LitEEGPTCausal(pl.LightningModule):
    ...
    def __init__(self, load_path="/users/yyang/EEGPT/EEGPT/checkpoint/eegpt_mcae_58chs_4s_large4E.ckpt"):
        ...
        self.chans_id = target_encoder.prepare_chan_ids(self.use_channels_names)
...
if __name__ == "__main__":
    ...
    for path in checkpoint_paths:
        ...
        use_channels_names = model.use_channels_names
        ...
        print("\n" + "="*70)
        print("SPATIAL PERTURBATION SENSITIVITY ANALYSIS - TUAB")
        print("="*70)
    
        # Print ROI information
        print("\nROI Definitions (based on 17 channels):")
            ...
            for noise_level in NOISE_LEVELS:
                ...
                with torch.no_grad():
                    for batch in test_loader:
                        ...
                        # Select the 17 channels first (before applying noise)
                        x = x[:, model.channel_indices, :]
                    
                        # Apply spatial noise to target region (on the 17 channels)
                        x = apply_spatial_noise(x, target_indices, noise_lambda=noise_level)
                    
                        # Forward pass (x is already 17 channels, so skip channel selection in forward)
                        B, C, T = x.shape
                        x_proc = x / 10
                        x_proc = model.chan_conv(x_proc)
                        z = model.target_encoder(x_proc, model.chans_id.to(x_proc))
                        h = z.flatten(2)
                        h = model.linear_probe1(model.drop(h))
                        h = h.flatten(1)
                        logits = model.linear_probe2(h)
                ...
                for i in range(len(per_class_acc)):
                    ...
                    print(f"      {class_name}: {per_class_acc[i]*100:.2f}% ({cm.sum(axis=1)[i]} samples)")
                print()
