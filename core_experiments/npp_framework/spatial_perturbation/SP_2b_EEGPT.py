...
def add_gaussian_noise(x, noise_level=1.0):
    ...
    batch_std = x.std()
    noise = torch.randn_like(x) * batch_std * noise_level
    x_perturbed = x + noise
    return x_perturbed

NOISE_LEVELS = [0.0, 0.5, 1.0, 2.0]  # Include 0.0 as baseline
...
for noise_level in NOISE_LEVELS:
    ...
    for i in range(1, 10):
        ...
        # Only need validation dataset for testing
        ...
        print(f"Subject {i:02d} ({current_subj}): Acc={accuracy:.4f} | F1={f1:.4f} | Kappa={kappa:.4f} | AUC-ROC={auroc:.4f}")
