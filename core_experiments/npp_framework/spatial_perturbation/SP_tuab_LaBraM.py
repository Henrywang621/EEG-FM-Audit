...
def load_labram_eval(ckpt_path):
    model = create_model(
        ...
        use_mean_pooling=True, use_rel_pos_bias=True, use_abs_pos_emb=True, qkv_bias=True, init_values=0.1,
    )
...
def run_analysis():
    ...
    loader = torch.utils.data.DataLoader(TUABLoader(DATA_PATH, files), batch_size=64, num_workers=4)
    ...
    for seed in SEEDS:
        ...
        set_seed(seed)
        model = load_labram_eval(os.path.join(BASE_DIR, f"seed_{seed}", "checkpoint-best.pth")).to(DEVICE)
        ...
        with torch.no_grad():
            for x, y in tqdm(loader, desc=f"Seed {seed} Inference"):
                ...
                p_orig.extend(torch.sigmoid(logits_o).cpu().numpy())

                for roi in REGIONS:
                    target_idx = get_roi_indices(roi, ch_names)
                    for n in NOISE_LEVELS:
                        x_p = apply_spatial_noise(x_in, target_idx, noise_lambda=n)
                        logits_p = model(x_p, input_chans=input_chans)
                        p_pert[roi][n].extend(torch.sigmoid(logits_p).cpu().numpy())

        yt = np.array(y_true).flatten()
        po = (np.array(p_orig).flatten() > 0.5).astype(float)
        ...
        print(f"{'ROI':<18} | {'Noise':<5} | {'BACC Diff':<10} | {'F1 Diff':<10} | {'Kappa Diff':<10}")
        print("-" * 65)

        for roi in REGIONS:
            for n in NOISE_LEVELS:
                pp = (np.array(p_pert[roi][n]).flatten() > 0.5).astype(float)
                m_p = {
                    "bacc": balanced_accuracy_score(yt, pp) * 100,
                    "f1": f1_score(yt, pp) * 100,
                ...
                print(f"{roi:<18} | {n:<5} | {m_orig['bacc']-m_p['bacc']:>9.2f}% | {m_orig['f1']-m_p['f1']:>9.2f}% | {m_orig['kappa']-m_p['kappa']:>10.4f}")

        del model
        torch.cuda.empty_cache()

    # ==========================================
    # 3. FINAL AGGREGATED SUMMARY
    # ==========================================
    metrics_to_print = [
    ...
    for title, key, fmt in metrics_to_print:
        ...
        for roi in REGIONS:
            for n in NOISE_LEVELS:
                p_m = np.mean(res_pert[roi][n][key])
                p_s = np.std(res_pert[roi][n][key])
                d_m = np.mean(res_diff[roi][n][key])
                d_s = np.std(res_diff[roi][n][key])
