...
# ==========================================
# 1. SPECTRAL ABLATION & MODEL LOADING
# ==========================================
...
def run_analysis():
    ...
    for seed in SEEDS:
        ...
        with torch.no_grad():
            for x, y in tqdm(loader, desc=f"Seed {seed} Inference"):
                ...
                for band in BANDS:
                    ...
                    p_bands[band].extend(torch.sigmoid(logits_a).cpu().numpy())

        # Seed Metric Calculation
        ...
        for band in BANDS:
            pa = (np.array(p_bands[band]).flatten() > 0.5).astype(float)
            m_a = {
