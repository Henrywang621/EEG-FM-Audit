...
def load_neurogpt_eval(ckpt_path, device):
    # num_decoding_classes = 6 for TUEV
    config = {
    ...
    for seed in MODEL_SEEDS:
        ...
        with torch.no_grad():
            for batch in tqdm(loader, desc=f"Seed {seed} Inference"):
                ...
                # 1. Original Baseline Prediction (Multi-class Argmax)
        ...
        # Calculate metrics for this seed (Weighted F1 for TUEV)
        yt = np.array(y_true)
        po = np.array(p_orig)
